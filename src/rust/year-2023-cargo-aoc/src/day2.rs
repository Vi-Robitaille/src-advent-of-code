...
lazy_static! {
    static ref GAME_REGEX: Regex = Regex::new(r"Game (?P<game_id>\d+):(?P<hand>( \d+ (red|blue|green),?)+;?)+").unwrap();
...
fn part_one(input: &Vec<Game>) -> usize {
    let constraints = Hand { red: 12, green: 13, blue: 14 };
    input
        .iter()
...
impl From<&str> for Game {
    ...
    fn from(value: &str) -> Self {
        ...
        let id = spl_game.next().unwrap().split(' ').nth(1).unwrap().parse::<usize>().unwrap();
        let dealings = spl_game.next().unwrap()
            .split(';')
            .map(|d| Hand::from(d))
            .collect::<Vec<Hand>>();
        Game { id, dealings }
...
impl Game {
    ...
    fn power_level(&self) -> usize {
        let bigliest_red = self.dealings.iter().max_by(|x, y| x.red.cmp(&y.red)).unwrap().red;
        let bigliest_green = self.dealings.iter().max_by(|x, y| x.green.cmp(&y.green)).unwrap().green;
        let bigliest_blue = self.dealings.iter().max_by(|x, y| x.blue.cmp(&y.blue)).unwrap().blue;
