...
class Monkey:
    ...
    def setup_monkeys(self, all_monkeys):
        self.true_target = all_monkeys[self.true_action]
        self.false_target = all_monkeys[self.false_action]
        self.global_mod = prod([x.test for x in all_monkeys])

    def inspect_items(self):
        self.inspected_items += len(self.held_items)
        self.held_items = [self.operation.calc(elem) % self.global_mod for elem in self.held_items]
...
def part1(input):
    ...
    [m.setup_monkeys(monkey_list) for m in monkey_list]
    for step in range(0, 10000):
        if step % 100 == 0:
            print(f"On step {step}")
        [m.do_turn() for m in monkey_list]
    [print(f"monkey: {i}, {x.inspected_items}") for i, x in enumerate(monkey_list)]
